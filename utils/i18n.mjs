 ...
 * * https://github.com/thunderbird/webext-examples/tree/master/manifest_v3/i18n
 *
 * Original license:
...
const getTranslationFromKey = (key, substitutions = []) => {
  let rv = _extension
    ? _extension.localeData.localizeMessage(key, substitutions)
    : messenger.i18n.getMessage(key, substitutions);
...
const updateDocument = (options = {}, sourceDocument) => {
  if (options) {
    if (options.extension) _extension = options.extension;
    if (options.keyPrefix) _keyPrefix = options.keyPrefix;
  }
  updateSubtree(sourceDocument, sourceDocument);
};

export function getTranslation(key, substitutions = []) {
  return getTranslationFromKey(key, substitutions);
}
